...
- candidates 배열의 값이 이동되지 않는 일 발생, 배열이 아닌 다른 방법을 탐색하는 것이 좋을 것 같음. n의 최적값을 구하는 코드는 구현하기 성공. 코드를 구현했으나 완벽하게 유전 알고리즘을 구현하지 못하여 a의 값을 찾을 수가 없었다. 또한, 코드를 구현할 때 교차율과 돌연변이율을 간과하고 있었는데 이를 구하기 위한 팁으로는 **교차율**을 80%~90%정도 높여야하며, **돌연변이율**은 0.5%~1% 정도로 매우 낮아야 한다.
